...
const Service: React.FC<IServiceProps> = inject("serviceStores")(
	observer(
		({ serviceStores }): React.ReactElement => {
			const itemRef = React.useRef<HTMLElement | any>(null)
			const [service, setService] = React.useState<boolean>(false)
			const [count, setCount] = React.useState<number>(0)
			const serviceBase: IService[] = serviceStores!.getServiceInfo
			let slidesType = null
			const onClickHandler = React.useCallback(() => {
				...
				serviceItem.forEach((item, id) => {
					item.addEventListener("click", () => {
						...
						slidesType =
							slide.current &&
							slide.current.querySelectorAll(".services_slider")
						...
						<div className='wrapper'>
							<div className='services_container'>
								...
								<div className='shift_description light_text'>
									Веб - студия International Techno Dynamics – надежный и
									опытный помощник в разработке сайтов любого вида. Мы готовы
									создать корпоративный сайт Вашей компании, с нуля разработать
									и запустить интернет-магазин, продумать в деталях эффективный
									landing, сделать все, чтобы сайт-визитка заинтересовал новых
									клиентов. Учитывая Ваши пожелания и идеи, мы разработаем
									web-дизайн для Вашего сайта. А для того, чтобы Ваш бизнес
									успешно развивался в сети, мы предлагаем грамотное продвижение
									Вашего сайта. Если Вы доверите нам свой проект, мы воплотим
									Вашу идею в жизнь.
								</div>
